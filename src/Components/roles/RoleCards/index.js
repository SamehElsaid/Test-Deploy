...
const RolesCards = ({ redirect, hidden, addMerchant }) => {
  ...
  const renderCards = () =>
    cardData.map((item, index) => (
      <Grid item xs={12} sm={6} lg={4} key={index}>
        <Card onClick={RedirectToMerchant}>
          <CardContent>
              ...
              <CardContent sx={{ pl: 0, height: '100%' }}>
                <Box sx={{ textAlign: 'right' }}>
                  ...
                  <Typography sx={{ color: 'text.secondary' }}>
                    {addMerchant
                      ? locale == 'ar'
                        ? 'أضف متجر إذا لم يكن موجودا'
                        : 'Add Merchant, if it does not exist.'
                      : locale == 'ar'
                      ? 'أضف دور إذا لم يكن موجودا'
